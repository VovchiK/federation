...
SpellMissInfo Unit::MeleeSpellHitResult(Unit *pVictim, SpellEntry const *spell, bool cMiss)
{
    ...
    if (canMiss)
    {
        ...
        // Roll miss
		tmp += spell->AttributesEx3 & SPELL_ATTR_EX3_CANT_MISS ? 0 : missChance;
        if (roll < tmp)
...
bool Unit::isSpellCrit(Unit *pVictim, SpellEntry const *spellProto, SpellSchoolMask schoolMask, WeaponAttackType attackType)
{
    ...
    switch(spellProto->DmgClass)
    {
        ...
        case SPELL_DAMAGE_CLASS_RANGED:
        {
            if (pVictim)
            {
                ...
                crit_chance+= (int32(GetMaxSkillValueForLevel(pVictim)) - int32(pVictim->GetDefenseSkillValue(this))) * 20.04f;
                crit_chance+= GetTotalAuraModifierByMiscMask(SPELL_AURA_MOD_SPELL_CRIT_CHANCE_SCHOOL, schoolMask);
            }
            break;
        }
    ...
    crit_chance = crit_chance > 20.0f ? crit_chance : 20.0f;
...
uint32 Unit::SpellCriticalBonus(SpellEntry const *spellProto, uint32 damage, Unit *pVictim)
{
    ...
    switch(spellProto->DmgClass)
    {
        ...
        case SPELL_DAMAGE_CLASS_RANGED:
            ...
            break;
        default:
            crit_bonus = damage / 7;                        // for spells is 50%
            break;
    }
