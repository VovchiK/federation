...
{
    float melee  = GetRatingBonusValue(CR_CRIT_TAKEN_MELEE)*100.0f;
    if (melee>250.0f) melee = 250.0f;
    return uint32 (melee * damage /1000.0f);
}

uint32 Player::GetRangedCritDamageReduction(uint32 damage) const
{
    float ranged = GetRatingBonusValue(CR_CRIT_TAKEN_RANGED)*100.0f;
    if (ranged>250.0f) ranged=250.0f;
    return uint32 (ranged * damage /1000.0f);
}
...
bool Player::RewardHonor(Unit *uVictim, uint32 groupsize, float honor, bool pvptoken)
{
	...
	if(uVictim && uVictim!=this && uVictim->GetTypeId()==TYPEID_PLAYER && GetSession()->GetRemoteAddress()==((Player*)uVictim)->GetSession()->GetRemoteAddress())
		return false;
